...
class Auth:
    ...
    def verify_password(self, plain_password, hashed_password):
        ...
        :param hashed_password: Compare the plain_password parameter with the hashed password stored in the database
    ...
    async def decode_refresh_token(self, refresh_token: str):
        ...
        The decode_refresh_token function is used to decode the refresh token.
            ...
            it will check if the scope of the payload is 'refresh_token'. If so, then we know that this
            is a valid refresh token, and we can return its email address (which was stored in sub).

        :param self: Represent the instance of the class
        :param refresh_token: str: Pass the refresh token to decode
        :return: The email of the user that is stored in the refresh_token
        :doc-author: Trelent
        """
        try:
            payload = jwt.decode(
    ...
    async def get_current_user(
        ...
        The get_current_user function is a dependency that will be used in the
            ...
            if it's valid, otherwise it raises an HTTPException with status code 401.
    ...
    def get_email_from_token(self, token: str):
        ...
        that token. It does this by decoding the JWT using our SECRET_KEY and ALGORITHM, then checking to make sure
        that it has a scope of 'email_token'. If so, it returns the email address from the payload's sub-field. If
        not, it raises an HTTPException with status code 401 (Unauthorized) and detail message &quot;Invalid scope
        for token&quot;. If there is any other error in decoding or validating the JWT, we raise another
